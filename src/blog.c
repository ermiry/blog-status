...
static void blog_env_get_runtime (void) {
...
static unsigned int blog_env_get_port (void) {
...
static void blog_env_get_cerver_receive_buffer_size (void) {
...
static void blog_env_get_cerver_th_threads (void) {
...
static void blog_env_get_cerver_connection_queue (void) {
...
static unsigned int blog_init_env (void) {

	unsigned int errors = 0;

	blog_env_get_runtime ();

	errors |= blog_env_get_port ();

	blog_env_get_cerver_receive_buffer_size ();

	blog_env_get_cerver_th_threads ();

	blog_env_get_cerver_connection_queue ();
...
unsigned int blog_init (void) {

	unsigned int errors = 0;

	if (!blog_init_env ()) {
		errors |= blog_service_init ();
	}
